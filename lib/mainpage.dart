...
import 'package:covid19apps/global/globalcountry.dart';
...
class _MainPageState extends State<MainPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('DATA COVID 19', style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1.0, fontSize: 23),),
        centerTitle: true,
              ...
              Container(
                ...
                child: Container(
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: <Widget>[
                      Text('DATA COVID (GLOBAL) PER ' + now.day.toString().toUpperCase() +" "+ bulan[now.month-1].toString().toUpperCase() + " " + now.year.toString(), style: TextStyle(fontWeight: FontWeight.bold),),
                      IconButton(icon: Icon(Icons.arrow_forward, color: Colors.blue,), onPressed: (){
                        Navigator.push(context, MaterialPageRoute(builder: (context)=> GlobalCountryPage()));
                      }),
                    ],
                  ),
                ),
                    ...
                    child: Column(
                      children: <Widget>[
                        Expanded(
                          child: Container(
                            child: FutureBuilder(
                              future: ConnectToAPI.GlobalDataAPI('https://api.kawalcorona.com/positif'),
                              builder: (context, snapshot){
                                if(snapshot.hasData){
                                  ...
                                  return ListView.builder(
                                    ...
                                    itemBuilder: (context, index){
                                      return Column(
                                        children: <Widget>[
                                          Text('KASUS POSITIF', style: TextStyle(letterSpacing: 2.0, fontWeight: FontWeight.bold),),
                                          SizedBox(height: 40,),
                                          Text(data.value, style: TextStyle(letterSpacing: 2.0, fontWeight: FontWeight.bold),)
                                        ],
                                      );
                                    },
                                  );
                    ...
                    child: Column(
                      children: <Widget>[
                        Expanded(
                          child: Container(
                            child: FutureBuilder(
                              future: ConnectToAPI.GlobalDataAPI('https://api.kawalcorona.com/sembuh'),
                              builder: (context, snapshot){
                                if(snapshot.hasData){
                                  Global data = snapshot.data;
                                  return ListView.builder(
                                    ...
                                    itemBuilder: (context, index){
                                      return Column(
                                        children: <Widget>[
                                          Text('SEMBUH', style: TextStyle(letterSpacing: 2.0, fontWeight: FontWeight.bold),),
                                          SizedBox(height: 40,),
                                          Text(data.value, style: TextStyle(letterSpacing: 2.0, fontWeight: FontWeight.bold),)
                                        ],
                                      );
                    ...
                    child: Column(
                      children: <Widget>[
                        Expanded(
                          child: Container(
                            child: FutureBuilder(
                              future: ConnectToAPI.GlobalDataAPI('https://api.kawalcorona.com/meninggal'),
                              builder: (context, snapshot){
                                if(snapshot.hasData){
                                  Global data = snapshot.data;
                                  return ListView.builder(
                                    ...
                                    itemBuilder: (context, index){
                                      return Column(
                                        children: <Widget>[
                                          Text('MENINGGAL', style: TextStyle(letterSpacing: 2.0, fontWeight: FontWeight.bold),),
                                          SizedBox(height: 40,),
                                          Text(data.value, style: TextStyle(letterSpacing: 2.0, fontWeight: FontWeight.bold),)
                                        ],
                                      );
